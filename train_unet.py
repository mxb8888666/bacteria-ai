# -*- coding: utf-8 -*-
"""
训练 U-Net 分割抑菌圈（完整可运行版）。

运行方式（先运行 make_unet_data.py 生成数据）：
  "D:/Program_Code/python学习库/.venv/Scripts/python.exe" train_unet.py

输出：
  unet_best.pt            最佳模型
  unet_training_curves.png 损失与 Dice 曲线
"""
...
def dice(pred, target):
    """Dice 系数：衡量分割重叠度，1=完全重合，是医学分割最常用的指标。"""
    pred = (pred > 0.5).float()
...
def main():
    ...
    criterion = nn.BCEWithLogitsLoss()   # 输出 logits，内部自带 sigmoid
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if vd > best_dice:                # 保存 Dice 最高的模型
            best_dice = vd
            torch.save(model.state_dict(), "unet_best.pt")

    print(f"\n最佳验证 Dice: {best_dice:.4f}（1.0=完美分割），模型已保存 unet_best.pt")
    ...
    plt.tight_layout(); plt.savefig("unet_training_curves.png", dpi=120)
